    ...
    if(req.query.accessUrl){
        ...
        switch(req.query.type){
            ...
            case 'block':
                ...
                break;
            default :
                Project.find({_id : req.query._id}).select('_id title brief accessUrl tags poster followers members owner localisation home trello progressIsTrue progress').populate('tags').execQ().then(function(data){
                    res.json(data);
                }).catch(function(err){
                    res.json(400,err);
                });
