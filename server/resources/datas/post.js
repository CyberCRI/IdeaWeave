...
switch (type) {
    ...
    case 'joinTeam':
        ...
        break;
    case 'login':
        dpd.users.get({ username : body.username },function(data){
            console.log(body);
            if(data.length > 0){
                if(data[0].emailValidated){
                    setResult(data[0]);
                }else{
                    cancel('you must validate your account check your emails',401);
                }
            }else{
                cancel('unknown username',401);
            }
        });
        break;
}
